...
var _sass = require('sass-wrapper');
...
var mod = {
    ...
    getSassCompiler: function(source) {
        return function(req, res, next) {
            ...
            // The SASS compiler seems to be compiling in 8K chunks, meaning
            // that this function will be called every time an 8K chunk has been
            // compiled. This also means that for larger compilation outputs,
            // this function could be invoked more than once.
            function chunkReady(err, data) {
                if (!err) {
                    // Since this function couldget called more than once, be
                    // sure to send headers just the first time.
                    if (!res.headersSent) {
                        res.writeHead(200, {
                            'Content-Type': 'text/css'
                        });
                    }
                    // Write whatever compilation output we have to the response
                    // stream.
                    res.write(data);

                    if (data.length < 8192) {
                        // If our data block was less than 8K, it must be the
                        // last chunk. End response.
                        res.end();
                    }
                } else {
            ...
            if (resourcePath.indexOf(source) === 0) {
                ...
                _fs.exists(scssFile, function(exists) {
                    if (exists) {
                        ...
                        _sass.compile({
                            filepath: resourcePath.replace(/.css$/, '.sass'),
                            callback: chunkReady
                        });
                    }
                });
